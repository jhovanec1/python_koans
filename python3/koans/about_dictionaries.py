...
class AboutDictionaries(Koan):
    def test_creating_dictionaries(self):
        ...
        self.assertEqual(True, len(empty_dict)==0)

    def test_dictionary_literals(self):
        empty_dict = {}
        self.assertEqual(dict, type(empty_dict))
        babel_fish = { 'one': 'uno', 'two': 'dos' }
        self.assertEqual(True, len(babel_fish)==2)

    def test_accessing_dictionaries(self):
        babel_fish = { 'one': 'uno', 'two': 'dos' }
        self.assertEqual(True, babel_fish['one']=='uno')
        self.assertEqual(True, babel_fish['two']=='dos')
    ...
    def test_dictionary_keys_and_values(self):
        babel_fish = {'one': 'uno', 'two': 'dos'}
        self.assertEqual(True, len(babel_fish.keys())==2)
        self.assertEqual(True, len(babel_fish.values())==2)
    ...
    def test_making_a_dictionary_from_a_sequence_of_keys(self):
        ...
        self.assertEqual(True, len(cards)==5)
        self.assertEqual(True, cards['green elf']==42)
        self.assertEqual(True, cards['yellow dwarf']==42)
